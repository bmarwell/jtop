...
public class AixProcessInfoServiceLoader implements ProcessInfoServiceLoader {

    @Override
    public boolean isOs(String currentOs) {
        return currentOs.toLowerCase(Locale.ROOT).startsWith("aix");
    }

    @Override
    public boolean isArch(String currentArch) {
        return "ppc64".equals(currentArch.toLowerCase(Locale.ROOT));
    }

    @Override
    public ProcessH newProcessH() {
        return new AixProcess();
    }
}
